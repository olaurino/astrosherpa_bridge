# -*- coding: utf-8 -*-
#
# asb documentation build configuration file, created by
...
# AstroPy Helpers (at present a hard-coded dependency that must be
...
#
asb_release = asb.__version__.split(".dev")[0]
...
# docs/ - so see if I can avoid this. Unfortunately, changing
# this adjusts both the location of where the files are added
# *and* the reference used by the toctree directive - e.g.
# if automodapi_toctreedirnm is chaned from 'api' to 'docs/api'
# then the files get written to 'docs/api/', but then the
# toctree directive is (depending on where the file is)
#    :toctree: docs/api/
#    :toctree: ../docs/api/
# so it appears there's no way to change it sensibly.
#
# It's also not clear that adding these files to docs/ is a good
# idea (without at least telling something somewhere to exclude
# this directory).
#
# automodapi_toctreedirnm = 'docs/api'
...
for n in ['pngmath', 'imgmath']:
    try:
        extensions.remove('sphinx.ext.' + n)
    except ValueError:
        pass
...
# the astropy_helpers sets up exclude_patterns to include _build,
# and I am going to assume that that is unlikely to change
...
# For when/if we use ReadTheDocs, based on
